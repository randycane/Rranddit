...
def subranddit_exists(form,field):
  ...
  subranddits = Subranddit.query.filter(Subranddit.title == subranddit).first()
  if subranddits:
...
class SubrandditForm(FlaskForm):
  title = StringField("name", validators=[DataRequired("Subranddit title is required!"), subranddit_exists, Length(min=3, max=30, message="Name length must be between 3 and 30 characters!"), Regexp('^\w+$', message="Subranddit name must contain only letters, numbers, underscores")])
